...
<template>
    <!-- Navbar Start -->
    <div class="px-5 sticky top-0 w-full z-50 bg-blue-500">
        <header class="flex justify-between items-center container mx-auto">
            ...
            <ul class="hidden md:flex space-x-10  font-medium items-center">
                <li class="uppercase hover:text-black text-white transition duration-200"><button
                        @click="$scrollTo('#home')">Tentang Kami</button></li>
                <li class="uppercase hover:text-black text-white transition duration-200"><button
                        @click="$scrollTo('#events')">Event</button></li>
                <li class="uppercase hover:text-black text-white transition duration-200"><button
                        @click="$scrollTo('#documentations')">Dokumentasi</button></li>
                <li class="uppercase hover:text-black text-white transition duration-200"><button
                        @click="$scrollTo('#sponsors')">Sponsor</button></li>
                <li class="uppercase hover:text-black text-white transition duration-200"><button
                        @click="$scrollTo('#contact')">Kontak</button></li>
            </ul>
        </header>

        <!-- Mobile Menu -->
        <transition name="slide-fade">
            <div v-if="mobileNav" class="md:hidden absolute top-16 left-0 w-full bg-blue-500 py-4 z-50">
